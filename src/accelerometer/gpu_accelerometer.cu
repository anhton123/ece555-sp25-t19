...
#define DEBUG (1)  // replace with value '1' to see accl and magnitude prints
...
// CUDA implementation to calculate magnitude
__global__
void gpu_compute_magnitude(accelerometer *accl,
		float *magnitude,
		int n) {
...
int main() 
{
	...
	float *magnitude;
	accelerometer *accl;
	float *cuda_magnitude;
	accelerometer *cuda_accl;
	...
	cudaMalloc((void**)&cuda_magnitude, num_samples * sizeof(float));
	cudaMalloc((void**)&cuda_accl, num_samples * sizeof(accelerometer));
	cudaMemcpy(cuda_accl, accl, num_samples * sizeof(accelerometer), cudaMemcpyHostToDevice);

	int blocks = (num_samples + 256 - 1) / 256;

	clock_gettime(CLOCK_MONOTONIC, &execution_start);
	gpu_compute_magnitude<<<blocks, 256>>>(cuda_accl, cuda_magnitude, num_samples);
	cudaDeviceSynchronize();
	clock_gettime(CLOCK_MONOTONIC, &execution_end);

	cudaMemcpy(magnitude, cuda_magnitude, num_samples * sizeof(float), cudaMemcpyDeviceToHost);
	...
	cudaFree(cuda_accl); cudaFree(cuda_magnitude);
